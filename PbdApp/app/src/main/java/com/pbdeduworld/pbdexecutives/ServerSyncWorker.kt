...
data class RequestObject(
    ...
    @SerializedName("lastReceiptReceivedAt") val lastReceiptReceivedAt: Long,
...
data class ReceiptDetailsResponseObject(
    @SerializedName("serverId") val serverId: String,
    ...
    private fun syncFunc(completer: CallbackToFutureAdapter.Completer<ListenableWorker.Result>) {
        GlobalScope.launch {
            ...
            if (apiKey != null) {            //if the user is not logged in then dont sync anything.
                ...
                //get receipt's last receivedAt
                val lastReceiptReceived: Receipts = db.receiptsDao().getLastReceviedAt()
                var lastReceiptReceivedAt: Long = 1

                if(lastReceiptReceived != null) {      //if the user is initially loading the data then Parties table wont ne there hence check for the null rule
                    lastReceiptReceivedAt = lastReceiptReceived.receivedAt
                }
                ...
                val requestJSONObject: JSONObject = JSONObject(Gson().toJson(RequestObject(
                    ...
                    lastReceiptReceivedAt = lastReceiptReceivedAt,
                    tasks = tasks,
                    followUps = followUps,
...
//                        Log.i("pbdLog", "responseObject: $responseObject")
                        ...
                        GlobalScope.launch {
                            ...
                            if(responseObject.receiptDetails != null) {
                                if(responseObject.receiptDetails.isNotEmpty()) {
                                    responseObject.receiptDetails.forEach { receiptDetails ->
                                        if(receiptDetails.serverId != null) {
                                            db.receiptsDao().markReceiptReceived(
                                                serverId = receiptDetails.serverId,
                                                receivedAt = receiptDetails.receivedAt
                                            )
                                        }
                                    }
                                }
